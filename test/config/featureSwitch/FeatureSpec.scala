...
class FeatureSpec extends PlaySpec with GuiceOneAppPerSuite with BeforeAndAfterEach {
  ...
  override def beforeEach(): Unit = {
    super.beforeEach()
    features.allowEventHubRequest(false)
  }

  "A feature" should {

    "return its current state" in {
      features.allowEventHubRequest() mustBe false
    }

    "switch to a new state" in {
      features.allowEventHubRequest(true)
      features.allowEventHubRequest() mustBe true
    }
  }
}
